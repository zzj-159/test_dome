...
loger = rep.Mylog
data = Interface_Data()
#放款状态，1:待放款,2:核算放款中,3:核算放款成功,4:核算放款失败,5:支付放款中,6:支付放款成功,7:支付放款失败,8:冲正中,9:已冲正
class Test_V2Loan_caxs:
    def setup_class(self):
        # self.loanId = rep.commonParam.set_flow_caxs(count=2,describe='测试账单中心新接口放款接口(/v2/loan)') #创建订单
    ...
    def test_1_01_正常放款一百万以下的小额放款_传入贷款金额小于100万_可以正常放款且数据库中数据正常落库_长安新生订单(self):
        pass
        loger.info("=====test_1_01_正常放款一百万以下的小额放款_传入贷款金额小于100万_可以正常放款且数据库中数据正常落库_长安新生订单=======接口测试开始")
        loger.info("创建订单")
        ...
        try:
            re = data.accounts_V2_loan(loanId=loanId,loanAmount=loanAmount,prodType=prod_type,clientNo=self.clientNo,userId=self.userId)
            loger.info("test_1_01_正常放款一百万以下的小额放款_传入贷款金额小于100万_可以正常放款且数据库中数据正常落库_长安新生订单======返回报文：%s" % (re.text))
            ...
            assert db_result[0]['loan_status'] in [5,6,4],loan_rsult
            ...
            if db_result2[0]['loan_status'] in [6]:
                data.accounts_V2_repay(loanId=loanId,userId=self.userId,clientNo=self.clientNo,
                                       receiptAmt=loanAmount,reqSeqNo="PLMR"+loanId)

        except (ZeroDivisionError, TypeError, NameError) as e:
            loger.error("test_1_01_正常放款一百万以下的小额放款_传入贷款金额小于100万_可以正常放款且数据库中数据正常落库_长安新生订单====断言失败，失败原因：%s" % (e))

    # @pytest.mark.skip(reason="数据原因")
    def test_1_02_正常放款一百万以上的大额放款_传入贷款金额大于100万_可以正常放款且数据库中数据正常落库_长安新生订单(self):
        # pass
        loger.info("=====test_1_02_正常放款一百万以上的大额放款_传入贷款金额大于100万_可以正常放款且数据库中数据正常落库_长安新生订单=======接口测试开始")
        loger.info("创建订单")
        loanId = rep.commonParam.set_flow_caxs(count=2, describe='测试账单中心新接口放款接口-正常放款一百万以上的小额放款(/v2/loan)')  # 创建订单
        loger.info("创建的订单号是：%s"%(loanId))
        print(loanId)
        loanAmount = 1200000
        prod_type = rep.commonParam.prod_type(prod_type_desc='长安新生').split(',')[0]
        try:
            re = data.accounts_V2_loan(loanId=loanId,loanAmount=loanAmount,prodType=prod_type,clientNo=self.clientNo,userId=self.userId)
            loger.info("test_1_02_正常放款一百万以上的大额放款_传入贷款金额大于100万_可以正常放款且数据库中数据正常落库_长安新生订单======返回报文：%s" % (re.text))
            assert re.status_code == 200
            res = re.json()
            print(res)
            assert res['code'] == 0
            ...
            loger.error("test_1_02_正常放款一百万以上的大额放款_传入贷款金额大于100万_可以正常放款且数据库中数据正常落库_长安新生订单====断言失败，失败原因：%s" % (e))

    # @pytest.mark.skip(reason="数据原因")
    def test_1_03_正常放款一百万_传入贷款金额等于100万_可以正常放款且数据库中数据正常落库_长安新生订单(self):
        # pass
        loger.info("=====test_1_02_正常放款一百万以上的大额放款_传入贷款金额大于100万_可以正常放款且数据库中数据正常落库_长安新生订单=======接口测试开始")
        loger.info("创建订单")
        loanId = rep.commonParam.set_flow_caxs(count=2, describe='测试账单中心新接口放款接口-正常放款一百万(/v2/loan)')  # 创建订单
        loger.info("创建的订单号是：%s"%(loanId))
        print(loanId)
        loanAmount = 1000000
        prod_type = rep.commonParam.prod_type(prod_type_desc='长安新生').split(',')[0]
    ...
    # @pytest.mark.skip(reason="数据原因")
    def test_1_04_放款金额校验_传入贷款金额为0_放款失败_长安新生订单(self):
        # pass
        loger.info("=====test_1_04_放款金额校验_传入贷款金额为0_放款失败_长安新生订单=======接口测试开始")
        loger.info("创建订单")
        loanId = rep.commonParam.set_flow_caxs(count=2, describe='测试账单中心新接口放款接口-放款金额为0(/v2/loan)')  # 创建订单
        loger.info("创建的订单号是：%s" % (loanId))
        print(loanId)
        loanAmount = 0
        prod_type = rep.commonParam.prod_type(prod_type_desc='长安新生').split(',')[0]
        try:
            re = data.accounts_V2_loan(loanId=loanId, loanAmount=loanAmount, prodType=prod_type, clientNo=self.clientNo,
                                       userId=self.userId)
            loger.info("test_1_04_放款金额校验_传入贷款金额为0_放款失败_长安新生订单======返回报文：%s" % (re.text))
            assert re.status_code == 200
            res = re.json()
            print(res)
            assert res['code'] == 0
            ...
            loger.error("test_1_04_放款金额校验_传入贷款金额为0_放款失败_长安新生订单====断言失败，失败原因：%s" % (e))

    # @pytest.mark.skip(reason="数据原因")
    def test_2_01_放款银行卡_对公放款_传入对公的账户信息_可以正常放款且数据库中数据正常落库_长安新生的订单(self):
        # pass
        loger.info("=====test_2_01_对公放款_传入对公的账户信息_可以正常放款且数据库中数据正常落库_长安新生的订单=======接口测试开始")
        loger.info("创建订单")
        loanId = rep.commonParam.set_flow_caxs(count=2, describe='测试账单中心新接口放款接口-对公放款(/v2/loan)')  # 创建订单
        loger.info("创建的订单号是：%s" % (loanId))
        print(loanId)
        loanAmount = 60000
        prod_type = rep.commonParam.prod_type(prod_type_desc='长安新生').split(',')[0]
        ...
        try:
            ...
            loger.info("test_2_01_对公放款_传入对公的账户信息_可以正常放款且数据库中数据正常落库_长安新生的订单======返回报文：%s" % (re.text))
            assert re.status_code == 200
            res = re.json()
            print(res)
            assert res['code'] == 0
            ...
            loger.error("test_2_01_对公放款_传入对公的账户信息_可以正常放款且数据库中数据正常落库_长安新生的订单====断言失败，失败原因：%s" % (e))

    # @pytest.mark.skip(reason="数据原因")
    def test_2_02_放款银行卡_对私放款_传入对私的账户信息_可以正常放款且数据库中数据正常落库_长安新生的订单(self):
        # pass
        loger.info("=====test_2_02_对私放款_传入对私的账户信息_可以正常放款且数据库中数据正常落库_长安新生的订单=======接口测试开始")
        loger.info("创建订单")
        loanId = rep.commonParam.set_flow_caxs(count=2, describe='测试账单中心新接口放款接口-对私放款(/v2/loan)')  # 创建订单
        loger.info("创建的订单号是：%s" % (loanId))
        print(loanId)
        loanAmount = 70000
        prod_type = rep.commonParam.prod_type(prod_type_desc='长安新生').split(',')[0]
        ...
        try:
            ...
            loger.info("test_2_02_对私放款_传入对私的账户信息_可以正常放款且数据库中数据正常落库_长安新生的订单======返回报文：%s" % (re.text))
            assert re.status_code == 200
            res = re.json()
            print(res)
            assert res['code'] == 0
            ...
            loger.error("test_2_02_对私放款_传入对私的账户信息_可以正常放款且数据库中数据正常落库_长安新生的订单====断言失败，失败原因：%s" % (e))

    # @pytest.mark.skip(reason="数据原因")
    def test_2_03_放款银行卡_银行卡账号异常_传入对私的错误的账户信息_放款失败报账号异常_长安新生的订单(self):
        # pass
        loger.info("=====test_2_03_放款银行卡_银行卡账号异常_传入对私的错误的账户信息_放款失败_长安新生的订单=======接口测试开始")
        loger.info("创建订单")
        loanId = rep.commonParam.set_flow_caxs(count=2, describe='测试账单中心新接口放款接口-银行卡异常(/v2/loan)')  # 创建订单
        loger.info("创建的订单号是：%s" % (loanId))
        print(loanId)
        loanAmount = 30000
        prod_type = rep.commonParam.prod_type(prod_type_desc='长安新生').split(',')[0]
        ...
        try:
            ...
            loger.info("test_2_03_放款银行卡_银行卡账号异常_传入对私的错误的账户信息_放款失败_长安新生的订单======返回报文：%s" % (re.text))
            assert re.status_code == 200
            res = re.json()
            print(res)
            assert res['code'] == 0
            ...
            loger.error("test_2_03_放款银行卡_银行卡账号异常_传入对私的错误的账户信息_放款失败_长安新生的订单====断言失败，失败原因：%s" % (e))

    # @pytest.mark.skip(reason="数据原因")
    def test_2_04_放款银行卡_银行卡账号异常_传入错误的对公的账户信息_放款失败报账号异常_长安新生的订单(self):
        # pass
        loger.info("=====test_2_04_放款银行卡_银行卡账号异常_传入对公的账户信息_放款失败_长安新生的订单=======接口测试开始")
        loger.info("创建订单")
        loanId = rep.commonParam.set_flow_caxs(count=2, describe='测试账单中心新接口放款接口-错误的对公放款(/v2/loan)')  # 创建订单
        loger.info("创建的订单号是：%s" % (loanId))
        print(loanId)
        loanAmount = 60000
        prod_type = rep.commonParam.prod_type(prod_type_desc='长安新生').split(',')[0]
        ...
        try:
            re = data.accounts_V2_loan(loanId=loanId, loanAmount=loanAmount, prodType=prod_type,clientNo=self.clientNo,userId=self.userId,
                    ...
                    payeeAcctType=public_account['payeeAcctType'],payeeCertType=public_account['payeeCertType'],
                                       payeeBankCode=public_account['payeeBankCode'])
            loger.info("test_2_04_放款银行卡_银行卡账号异常_传入对公的账户信息_放款失败_长安新生的订单======返回报文：%s" % (re.text))
            assert re.status_code == 200
            res = re.json()
            print(res)
            assert res['code'] == 0
            ...
            loger.error("test_2_04_放款银行卡_银行卡账号异常_传入对公的账户信息_放款失败_长安新生的订单====断言失败，失败原因：%s" % (e))

    # @pytest.mark.skip(reason="数据原因")
    def test_2_05_放款银行卡_行内银行卡放款_传入锡商银行卡号_可以正常放款且数据库中数据正常落库_长安新生的订单(self):
        # pass
        loger.info("=====test_2_05_放款银行卡_行内银行卡放款_传入锡商银行卡号_可以正常放款且数据库中数据正常落库_长安新生的订单=======接口测试开始")
        loger.info("创建订单")
        loanId = rep.commonParam.set_flow_caxs(count=2, describe='测试账单中心新接口放款接口-行内放款(/v2/loan)')  # 创建订单
        loger.info("创建的订单号是：%s" % (loanId))
        print(loanId)
        loanAmount = 70000
        prod_type = rep.commonParam.prod_type(prod_type_desc='长安新生').split(',')[0]
        ...
        try:
            re = data.accounts_V2_loan(loanId=loanId, loanAmount=loanAmount, prodType=prod_type,clientNo=self.clientNo,userId=self.userId,
                                       ...
                                       payeeAcctType=private_account['payeeAcctType'],
                                       payeeCertType=private_account['payeeCertType'])
            loger.info("test_2_05_放款银行卡_行内银行卡放款_传入锡商银行卡号_可以正常放款且数据库中数据正常落库_长安新生的订单======返回报文：%s" % (re.text))
            assert re.status_code == 200
            res = re.json()
            print(res)
            assert res['code'] == 0
            ...
            loger.error("test_2_05_放款银行卡_行内银行卡放款_传入锡商银行卡号_可以正常放款且数据库中数据正常落库_长安新生的订单====断言失败，失败原因：%s" % (e))

    # @pytest.mark.skip(reason="数据原因")
    def test_3_01_核算成功放款失败状态进行重新放款_传入因对公的账户信息错误导致放款失败订单号_修改成正确的银行卡信息进行重新放款_重新放款成功且数据库银行卡信息修改正确_长安新生的订单(self):
        # pass
        loger.info("=====test_3_01_核算成功放款失败状态进行重新放款_传入因对公的账户信息错误导致放款失败订单号_修改成正确"
                   "的银行卡信息进行重新放款_重新放款成功且数据库银行卡信息修改正确_长安新生的订单=======接口测试开始")
        loger.info("创建订单")
        loanId = rep.commonParam.set_flow_caxs(count=2, describe='测试账单中心新接口放款接口-错误的对公放款重新放款(/v2/loan)')  # 创建订单
        loger.info("创建的订单号是：%s" % (loanId))
        ...
        try:
            ...
            loger.info("test_3_01_核算成功放款失败状态进行重新放款_传入因对公的账户信息错误导致放款失败订单号_修改成正确"
                       "的银行卡信息进行重新放款_重新放款成功且数据库银行卡信息修改正确_长安新生的订单======返回报文：%s" % (re.text))
            assert re.status_code == 200
            res = re.json()
            print(res)
            assert res['code'] == 0
            ...
            if db_result[0]['loan_status'] == 7:
                print("============开始进行重新放款===================")
                loger.info("============开始进行重新放款===================")
                reloan = data.accounts_V2_loan(loanId=loanId, loanAmount=loanAmount, prodType=prod_type,clientNo=self.clientNo,userId=self.userId,
            ...
            #     db_result = [{'id':1245}]
                ...
                for dbre in db_result_reloan:
                    ...
                    if dbre['id'] != db_result[0]['id']:
                        ...
                        if dbre['loan_status'] in [2, 5, 6,4]:
                            data.mock_accounts(loanid=loanId, uppStatus="00")
                            data.accounts_V2_repay(loanId=loanId, userId=self.userId, clientNo=self.clientNo,
                                               receiptAmt=loanAmount, reqSeqNo="PLMR" + loanId)
    ...
    # @pytest.mark.skip(reason="数据原因")
    def test_3_02_核算成功放款失败状态进行重新放款_传入因对私的账户信息错误导致放款失败订单号_修改成正确的银行卡信息进行重新放款_重新放款成功且数据库银行卡信息修改正确_长安新生的订单(self):
        # pass
        loger.info("=====test_3_02_核算成功放款失败状态进行重新放款_传入因对私的账户信息错误导致放款失败订单号_修改成正确的"
                   "银行卡信息进行重新放款_重新放款成功且数据库银行卡信息修改正确_长安新生的订单=======接口测试开始")
        loger.info("创建订单")
        loanId = rep.commonParam.set_flow_caxs(count=2, describe='测试账单中心新接口放款接口-错误的对私放款重新放款(/v2/loan)')  # 创建订单
        loger.info("创建的订单号是：%s" % (loanId))
        print("创建的订单号是：",loanId)
        loanAmount = 30000
        prod_type = rep.commonParam.prod_type(prod_type_desc='长安新生').split(',')[0]
        ...
        try:
            ...
            loger.info("test_3_02_核算成功放款失败状态进行重新放款_传入因对私的账户信息错误导致放款失败订单号_修改成正确的"
                       "银行卡信息进行重新放款_重新放款成功且数据库银行卡信息修改正确_长安新生的订单======返回报文：%s" % (re.text))
            assert re.status_code == 200
            res = re.json()
            print(res)
            assert res['code'] == 0
            ...
            #     db_result = [{'id':1245}]
                ...
                for dbre in db_result_reloan:
                    ...
                    if dbre['id'] != db_result[0]['id']:
                        ...
                        print("银行卡验证完成，一切正常")
                        if dbre['loan_status'] in [5, 6]:
                            data.mock_accounts(loanid=loanId, uppStatus="00")
                            data.accounts_V2_repay(loanId=loanId, userId=self.userId, clientNo=self.clientNo,
                                                   receiptAmt=loanAmount, reqSeqNo="PLMR" + loanId)
        except (ZeroDivisionError, TypeError, NameError) as e:
            loger.error("test_3_02_核算成功放款失败状态进行重新放款_传入因对私的账户信息错误导致放款失败订单号_修改成正确"
                      "的银行卡信息进行重新放款_重新放款成功且数据库银行卡信息修改正确_长安新生的订单====断言失败，失败原因：%s" % (e))

    # @pytest.mark.skip(reason="数据原因")
    def test_3_03_核算成功放款失败状态进行重新放款_传入的账户信息错误导致放款失败订单号_依旧修改为错误的银行卡信息进行重新放款_重新放款失败_长安新生的订单(self):
        # pass
        loger.info("=====test_3_03_核算成功放款失败状态进行重新放款_传入因对私的账户信息错误导致放款失败订单号_依旧修改为"
                   "错误的银行卡信息进行重新放款_重新放款失_长安新生的订单=======接口测试开始")
        loger.info("创建订单")
        loanId = rep.commonParam.set_flow_caxs(count=2, describe='对私重新放款失败(/v2/loan)')  # 创建订单
        loger.info("创建的订单号是：%s" % (loanId))
        print("创建的订单号是：", loanId)
        loanAmount = 30000
        prod_type = rep.commonParam.prod_type(prod_type_desc='长安新生').split(',')[0]
        ...
        try:
            ...
            loger.info("test_3_03_核算成功放款失败状态进行重新放款_传入因对私的账户信息错误导致放款失"
                       "败订单号_依旧修改为错误的银行卡信息进行重新放款_重新放款失_长安新生的订单======返回报文：%s" % (re.text))
            assert re.status_code == 200
            res = re.json()
            print(res)
            assert res['code'] == 0
            ...
            loger.error("test_3_03_核算成功放款失败状态进行重新放款_传入因对私的账户信息错误导致放款失败"
                        "订单号_依旧修改为错误的银行卡信息进行重新放款_重新放款失_长安新生的订单====断言失败，失败原因：%s" % (e))

    # @pytest.mark.skip(reason="数据原因")
    # def test_4_01_还款方式不同的放款_还款方式为_1_等额本息_放款成功_长安新生订单(self):
    #     pass
    #     loger.info("=====test_4_01_还款方式不同的放款_还款方式为_1_等额本息_放款成功_长安新生订单=======接口测试开始")
    #     loger.info("创建订单")
    #     loanId = rep.commonParam.set_flow_caxs(count=2, describe='测试账单中心新接口放款接口-还款方式为1的放款(/v2/loan)')  # 创建订单
    #     loger.info("创建的订单号是：%s" % (loanId))
    #     print("创建的订单号是：", loanId)
    #     loanAmount = 30000
    #     prod_type = rep.commonParam.prod_type(prod_type_desc='长安新生').split(',')[0]
    ...
    #         loger.info("test_4_01_还款方式不同的放款_还款方式为_1_等额本息_放款成功_长安新生订单======返回报文：%s" % (re.text))
    #         assert re.status_code == 200
    #         res = re.json()
    #         print(res)
    #         assert res['code'] == 0
    ...
    #         assert res['result'] == True
    #     except (ZeroDivisionError, TypeError, NameError) as e:
    #         loger.error("test_4_01_还款方式不同的放款_还款方式为_1_等额本息_放款成功_长安新生订单====断言失败，失败原因：%s" % (e))

    # @pytest.mark.skip(reason="数据原因")
    # def test_4_02_还款方式不同的放款_还款方式为_2_等额本金_放款成功_长安新生订单(self):
    #     pass
    #     loger.info("=====test_4_02_还款方式不同的放款_还款方式为_2_等额本金_放款成功_长安新生订单=======接口测试开始")
    #     loger.info("创建订单")
    #     loanId = rep.commonParam.set_flow_caxs(count=2, describe='测试账单中心新接口放款接口-还款方式为2的放款(/v2/loan)')  # 创建订单
    #     loger.info("创建的订单号是：%s" % (loanId))
    #     print("创建的订单号是：", loanId)
    #     loanAmount = 20000
    #     prod_type = rep.commonParam.prod_type(prod_type_desc='长安新生').split(',')[0]
    ...
    #         re = data.accounts_V2_loan(loanId=loanId, loanAmount=loanAmount, prodType=prod_type,clientNo=self.clientNo,
    #                                    userId=self.userId,repayMode=repayMode)
    #         loger.info("test_4_02_还款方式不同的放款_还款方式为_2_等额本金_放款成功_长安新生订单======返回报文：%s" % (re.text))
    #         assert re.status_code == 200
    #         res = re.json()
    #         print(res)
    #         assert res['code'] == 0
    ...
    #         assert res['result'] == True
    #     except (ZeroDivisionError, TypeError, NameError) as e:
    #         loger.error("test_4_02_还款方式不同的放款_还款方式为_2_等额本金_放款成功_长安新生订单====断言失败，失败原因：%s" % (e))

    # @pytest.mark.skip(reason="数据原因")
    # def test_4_01_接口必填参数校验_传入空订单号_接口调用失败_借据号不能为空_长安新生订单(self):
    #     loger.info("=====test_5_01_接口必填参数校验_传入空订单号_接口调用失败_借据号不能为空_长安新生订单=======接口测试开始")
    #     # loger.info("创建订单")
    #     # loanId = rep.commonParam.set_flow_caxs(count=2, describe='测试账单中心新接口放款接口-正常放款一百万以下的小额放款(/v2/loan)')  # 创建订单
    #     # loger.info("创建的订单号是：%s" % (loanId))
    #     # print("创建的订单号：", loanId)
    ...
    #         re = data.accounts_V2_loan(loanId=loanId, loanAmount=loanAmount, prodType=prod_type, clientNo=self.clientNo,
    #                                    userId=self.userId)
    #         loger.info("test_5_01_接口必填参数校验_传入空订单号_接口调用失败_借据号不能为空_长安新生订单======返回报文：%s" % (re.text))
    #         assert re.status_code == 200
    #         res = re.json()
    #         print(res)
    #         assert res['code'] != 0
    ...
    #
    # @pytest.mark.skip(reason="数据原因")
    # def test_4_02_接口必填参数校验_传入空userId_接口调用失败_用户id不能为空_长安新生订单(self):
    #     loger.info("=====test_5_02_接口必填参数校验_传入空userId_接口调用失败_用户id不能为空_长安新生订单=======接口测试开始")
    #     loger.info("创建订单")
    #     loanId = rep.commonParam.set_flow_caxs(count=2, describe='测试账单中心新接口放款接口-正常放款一百万以下的小额放款(/v2/loan)')  # 创建订单
    #     loger.info("创建的订单号是：%s" % (loanId))
    #     print("创建的订单号：", loanId)
    ...
    #         re = data.accounts_V2_loan(loanId=loanId, loanAmount=loanAmount, prodType=prod_type, clientNo=self.clientNo,
    #                                    userId='')
    #         loger.info("test_5_02_接口必填参数校验_传入空userId_接口调用失败_用户id不能为空_长安新生订单======返回报文：%s" % (re.text))
    #         assert re.status_code == 200
    #         res = re.json()
    #         print(res)
    #         assert res['code'] != 0
    ...
    #
    #     except (ZeroDivisionError, TypeError, NameError) as e:
    #         loger.error("test_5_02_接口必填参数校验_传入空userId_接口调用失败_用户id不能为空_长安新生订单====断言失败，失败原因：%s" % (e))
    #
    # @pytest.mark.skip(reason="数据原因")
    # def test_5_03_接口必填参数校验_传入空clientNo_接口调用失败_clientNo不能为空_长安新生订单(self):
    #     pass
    #     loger.info("=====test_5_03_接口必填参数校验_传入空clientNo_接口调用失败_clientNo不能为空_长安新生订单=======接口测试开始")
    #     loger.info("创建订单")
    #     loanId = rep.commonParam.set_flow_caxs(count=2, describe='测试账单中心新接口放款接口-正常放款一百万以下的小额放款(/v2/loan)')  # 创建订单
    #     loger.info("创建的订单号是：%s" % (loanId))
    #     print("创建的订单号：", loanId)
    ...
    #     prod_type = rep.commonParam.prod_type(prod_type_desc='长安新生').split(',')[0]
    #     try:
    #         re = data.accounts_V2_loan(loanId=loanId, loanAmount=loanAmount, prodType=prod_type, clientNo='',
    #                                    userId=self.userId)
    #         loger.info("test_5_03_接口必填参数校验_传入空clientNo_接口调用失败_clientNo不能为空_长安新生订单======返回报文：%s" % (re.text))
    #         assert re.status_code == 200
    #         res = re.json()
    #         print(res)
    #         assert res['code'] != 0
    ...
    #
    #     except (ZeroDivisionError, TypeError, NameError) as e:
    #         loger.error("test_5_03_接口必填参数校验_传入空clientNo_接口调用失败_clientNo不能为空_长安新生订单====断言失败，失败原因：%s" % (e))