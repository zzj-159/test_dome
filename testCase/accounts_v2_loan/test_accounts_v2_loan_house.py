...
class Test_V2Loan_house:
    ...
    def test_1_01_正常放款一百万以下的小额放款_传入贷款金额小于100万_可以正常放款且数据库中数据正常落库_锡房贷订单(self):
        # pass
        loger.info("=====test_1_01_正常放款一百万以下的小额放款_传入贷款金额小于100万_可以正常放款且数据库中数据正常落库_锡房贷订单=======接口测试开始")
        loger.info("创建订单")
        loanId = rep.commonParam.set_flow(count=2, describe='测试账单中心新接口放款接口-正常放款一百万以下的小额放款(/v2/loan)')  # 创建订单
        loger.info("创建的订单号是：%s"%(loanId))
        print("创建的订单号：",loanId)
        loanAmount = 700000
        prod_type = rep.commonParam.prod_type(prod_type_desc='安居客')
        print(prod_type)
        try:
            re = data.accounts_V2_loan(loanId=loanId,loanAmount=loanAmount,prodType=prod_type,clientNo=self.clientNo,
                                       userId=self.userId,partnerId='AJK')
            loger.info("test_1_01_正常放款一百万以下的小额放款_传入贷款金额小于100万_可以正常放款且数据库中数据正常落库_锡房贷订单======返回报文：%s" % (re.text))
            ...
            loger.error("test_1_01_正常放款一百万以下的小额放款_传入贷款金额小于100万_可以正常放款且数据库中数据正常落库_锡房贷订单====断言失败，失败原因：%s" % (e))

    # @pytest.mark.skip(reason="数据原因")
    def test_1_02_正常放款一百万以上的大额放款_传入贷款金额大于100万_可以正常放款且数据库中数据正常落库_锡房贷订单(self):

        loger.info("=====test_1_02_正常放款一百万以上的大额放款_传入贷款金额大于100万_可以正常放款且数据库中数据正常落库_锡房贷订单=======接口测试开始")
        loger.info("创建订单")
        ...
        try:
            re = data.accounts_V2_loan(loanId=loanId,loanAmount=loanAmount,prodType=prod_type,clientNo=self.clientNo,
                                       userId=self.userId,partnerId='AJK')
            loger.info("test_1_02_正常放款一百万以上的大额放款_传入贷款金额大于100万_可以正常放款且数据库中数据正常落库_锡房贷订单======返回报文：%s" % (re.text))
            assert re.status_code == 200
            res = re.json()
            print(res)
            assert res['code'] == 0
            ...
            loger.error("test_1_02_正常放款一百万以上的大额放款_传入贷款金额大于100万_可以正常放款且数据库中数据正常落库_锡房贷订单====断言失败，失败原因：%s" % (e))

    # @pytest.mark.skip(reason="数据原因")
    def test_1_03_正常放款一百万_传入贷款金额等于100万_可以正常放款且数据库中数据正常落库_锡房贷订单(self):
        # pass
        loger.info("=====test_1_02_正常放款一百万以上的大额放款_传入贷款金额大于100万_可以正常放款且数据库中数据正常落库_锡房贷订单=======接口测试开始")
        loger.info("创建订单")
    ...
    # @pytest.mark.skip(reason="数据原因")
    def test_1_04_放款金额校验_传入贷款金额为0_放款失败_锡房贷订单(self):
        pass
        loger.info("=====test_1_04_放款金额校验_传入贷款金额为0_放款失败_锡房贷订单=======接口测试开始")
        loger.info("创建订单")
        ...
        try:
            re = data.accounts_V2_loan(loanId=loanId, loanAmount=loanAmount, prodType=prod_type, clientNo=self.clientNo,
                                       userId=self.userId,partnerId='AJK')
            loger.info("test_1_04_放款金额校验_传入贷款金额为0_放款失败_锡房贷订单======返回报文：%s" % (re.text))
            assert re.status_code == 200
            res = re.json()
            print(res)
            assert res['code'] == 0
            ...
            loger.error("test_1_04_放款金额校验_传入贷款金额为0_放款失败_锡房贷订单====断言失败，失败原因：%s" % (e))

    # @pytest.mark.skip(reason="数据原因")
    def test_2_01_放款银行卡_对公放款_传入对公的账户信息_可以正常放款且数据库中数据正常落库_锡房贷订单(self):
        # pass
        loger.info("=====test_2_01_对公放款_传入对公的账户信息_可以正常放款且数据库中数据正常落库_锡房贷订单=======接口测试开始")
        loger.info("创建订单")
        ...
        try:
            re = data.accounts_V2_loan(loanId=loanId,partnerId='AJK',loanAmount=loanAmount, prodType=prod_type,clientNo=self.clientNo,userId=self.userId,
            ...
            loger.info("test_2_01_对公放款_传入对公的账户信息_可以正常放款且数据库中数据正常落库_锡房贷订单======返回报文：%s" % (re.text))
            assert re.status_code == 200
            res = re.json()
            print(res)
            assert res['code'] == 0
            ...
            loger.error("test_2_01_对公放款_传入对公的账户信息_可以正常放款且数据库中数据正常落库_锡房贷订单====断言失败，失败原因：%s" % (e))

    # @pytest.mark.skip(reason="数据原因")
    def test_2_02_放款银行卡_对私放款_传入对私的账户信息_可以正常放款且数据库中数据正常落库_锡房贷订单(self):
        pass
        loger.info("=====test_2_02_对私放款_传入对私的账户信息_可以正常放款且数据库中数据正常落库_锡房贷订单=======接口测试开始")
        loger.info("创建订单")
        ...
        private_account={"payeeAccDept":"邮政储蓄银行","payeeAcctName":"张正杰","payeeAcctNo":"6221885200197319710",
        ...
        try:
            ...
            loger.info("test_2_02_对私放款_传入对私的账户信息_可以正常放款且数据库中数据正常落库_锡房贷订单======返回报文：%s" % (re.text))
            assert re.status_code == 200
            res = re.json()
            print(res)
            assert res['code'] == 0
            ...
            loger.error("test_2_02_对私放款_传入对私的账户信息_可以正常放款且数据库中数据正常落库_锡房贷订单====断言失败，失败原因：%s" % (e))

    # @pytest.mark.skip(reason="数据原因")
    def test_2_03_放款银行卡_银行卡账号异常_传入对私的错误的账户信息_放款失败报账号异常_锡房贷订单(self):
        pass
        loger.info("=====test_2_03_放款银行卡_银行卡账号异常_传入对私的错误的账户信息_放款失败_锡房贷订单=======接口测试开始")
        loger.info("创建订单")
        ...
        try:
            ...
            loger.info("test_2_03_放款银行卡_银行卡账号异常_传入对私的错误的账户信息_放款失败_锡房贷订单======返回报文：%s" % (re.text))
            assert re.status_code == 200
            res = re.json()
            print(res)
            assert res['code'] == 0
            ...
            loger.error("test_2_03_放款银行卡_银行卡账号异常_传入对私的错误的账户信息_放款失败_锡房贷订单====断言失败，失败原因：%s" % (e))

    # @pytest.mark.skip(reason="数据原因")
    def test_2_04_放款银行卡_银行卡账号异常_传入错误的对公的账户信息_放款失败报账号异常_锡房贷订单(self):
        # pass
        loger.info("=====test_2_04_放款银行卡_银行卡账号异常_传入对公的账户信息_放款失败_锡房贷订单=======接口测试开始")
        loger.info("创建订单")
        ...
        try:
            re = data.accounts_V2_loan(loanId=loanId, partnerId='AJK',loanAmount=loanAmount, prodType=prod_type,clientNo=self.clientNo,userId=self.userId,
                    ...
                    payeeAcctType=public_account['payeeAcctType'],payeeCertType=public_account['payeeCertType'],
                                       payeeBankCode=public_account['payeeBankCode'])
            loger.info("test_2_04_放款银行卡_银行卡账号异常_传入对公的账户信息_放款失败_锡房贷订单======返回报文：%s" % (re.text))
            assert re.status_code == 200
            res = re.json()
            print(res)
            assert res['code'] == 0
            ...
            loger.error("test_2_04_放款银行卡_银行卡账号异常_传入对公的账户信息_放款失败_锡房贷订单====断言失败，失败原因：%s" % (e))

    # @pytest.mark.skip(reason="数据原因")
    def test_2_05_放款银行卡_行内银行卡放款_传入锡商银行卡号_可以正常放款且数据库中数据正常落库_锡房贷订单(self):
        # pass
        loger.info("=====test_2_05_放款银行卡_行内银行卡放款_传入锡商银行卡号_可以正常放款且数据库中数据正常落库_锡房贷订单=======接口测试开始")
        loger.info("创建订单")
        ...
        try:
            re = data.accounts_V2_loan(loanId=loanId, partnerId='AJK',loanAmount=loanAmount, prodType=prod_type,clientNo=self.clientNo,userId=self.userId,
                                       ...
                                       payeeAcctType=private_account['payeeAcctType'],
                                       payeeCertType=private_account['payeeCertType'])
            loger.info("test_2_05_放款银行卡_行内银行卡放款_传入锡商银行卡号_可以正常放款且数据库中数据正常落库_锡房贷订单======返回报文：%s" % (re.text))
            assert re.status_code == 200
            res = re.json()
            print(res)
            assert res['code'] == 0
            ...
            loger.error("test_2_05_放款银行卡_行内银行卡放款_传入锡商银行卡号_可以正常放款且数据库中数据正常落库_锡房贷订单====断言失败，失败原因：%s" % (e))

    # @pytest.mark.skip(reason="数据原因")
    def test_3_01_核算成功放款失败状态进行重新放款_传入因对公的账户信息错误导致放款失败订单号_修改成正确的银行卡信息进行重新放款_重新放款成功且数据库银行卡信息修改正确_锡房贷订单(self):
        pass
        loger.info("=====test_3_01_核算成功放款失败状态进行重新放款_传入因对公的账户信息错误导致放款失败订单号_修改成正确"
                   "的银行卡信息进行重新放款_重新放款成功且数据库银行卡信息修改正确_锡房贷订单=======接口测试开始")
        loger.info("创建订单")
        ...
        try:
            ...
            loger.info("test_3_01_核算成功放款失败状态进行重新放款_传入因对公的账户信息错误导致放款失败订单号_修改成正确"
                       "的银行卡信息进行重新放款_重新放款成功且数据库银行卡信息修改正确_锡房贷订单======返回报文：%s" % (re.text))
            assert re.status_code == 200
            res = re.json()
            print(res)
            assert res['code'] == 0
            ...
            if db_result[0]['loan_status'] == 7:
                print("============开始进行重新放款===================")
                loger.info("============开始进行重新放款===================")
                reloan = data.accounts_V2_loan(loanId=loanId,partnerId='AJK', loanAmount=loanAmount, prodType=prod_type,clientNo=self.clientNo,userId=self.userId,
    ...
    # @pytest.mark.skip(reason="数据原因")
    def test_3_02_核算成功放款失败状态进行重新放款_传入因对私的账户信息错误导致放款失败订单号_修改成正确的银行卡信息进行重新放款_重新放款成功且数据库银行卡信息修改正确_锡房贷订单(self):
        # pass
        loger.info("=====test_3_02_核算成功放款失败状态进行重新放款_传入因对私的账户信息错误导致放款失败订单号_修改成正确的"
                   "银行卡信息进行重新放款_重新放款成功且数据库银行卡信息修改正确_锡房贷订单=======接口测试开始")
        loger.info("创建订单")
        ...
        try:
            ...
            loger.info("test_3_02_核算成功放款失败状态进行重新放款_传入因对私的账户信息错误导致放款失败订单号_修改成正确的"
                       "银行卡信息进行重新放款_重新放款成功且数据库银行卡信息修改正确_锡房贷订单======返回报文：%s" % (re.text))
            assert re.status_code == 200
            res = re.json()
            print(res)
            assert res['code'] == 0
            ...
            loger.error("test_3_02_核算成功放款失败状态进行重新放款_传入因对私的账户信息错误导致放款失败订单号_修改成正确"
                        "的银行卡信息进行重新放款_重新放款成功且数据库银行卡信息修改正确_锡房贷订单====断言失败，失败原因：%s" % (e))

    # @pytest.mark.skip(reason="数据原因")
    def test_3_03_核算成功放款失败状态进行重新放款_传入的账户信息错误导致放款失败订单号_依旧修改为错误的银行卡信息进行重新放款_重新放款失败_锡房贷订单(self):
        # pass
        loger.info("=====test_3_03_核算成功放款失败状态进行重新放款_传入因对私的账户信息错误导致放款失败订单号_依旧修改为"
                   "错误的银行卡信息进行重新放款_重新放款失_锡房贷订单=======接口测试开始")
        loger.info("创建订单")
        ...
        try:
            ...
            loger.info("test_3_03_核算成功放款失败状态进行重新放款_传入因对私的账户信息错误导致放款失"
                       "败订单号_依旧修改为错误的银行卡信息进行重新放款_重新放款失_锡房贷订单======返回报文：%s" % (re.text))
            assert re.status_code == 200
            res = re.json()
            print(res)
            assert res['code'] == 0
            ...
            loger.error("test_3_03_核算成功放款失败状态进行重新放款_传入因对私的账户信息错误导致放款失败"
                        "订单号_依旧修改为错误的银行卡信息进行重新放款_重新放款失_锡房贷订单====断言失败，失败原因：%s" % (e))

    # @pytest.mark.skip(reason="数据原因")
    # def test_4_01_还款方式不同的放款_还款方式为_1_等额本息_放款成功_安居客订单(self):
    #     pass
    # #     loger.info("=====test_4_01_还款方式不同的放款_还款方式为_1_等额本息_放款成功_安居客订单=======接口测试开始")
    # #     loger.info("创建订单")
    ...
    # #     repayMode = "1"
    # #     try:
    # #         re = data.accounts_V2_loan(loanId=loanId,partnerId='AJK', loanAmount=loanAmount, prodType=prod_type,clientNo=self.clientNo,
    # #                                    userId=self.userId,repayMode=repayMode)
    # #         loger.info("test_4_01_还款方式不同的放款_还款方式为_1_等额本息_放款成功_安居客订单======返回报文：%s" % (re.text))
    # #         assert re.status_code == 200
    # #         res = re.json()
    # #         print(res)
    # #         assert res['code'] == 0
    ...
    # #         assert res['result'] == True
    # #     except (ZeroDivisionError, TypeError, NameError) as e:
    # #         loger.error("test_4_01_还款方式不同的放款_还款方式为_1_等额本息_放款成功_安居客订单====断言失败，失败原因：%s" % (e))
    #
    # @pytest.mark.skip(reason="数据原因")
    # def test_4_02_还款方式不同的放款_还款方式为_2_等额本金_放款成功_安居客订单(self):
    #     pass
    # #     loger.info("=====test_4_02_还款方式不同的放款_还款方式为_2_等额本金_放款成功_安居客订单=======接口测试开始")
    # #     loger.info("创建订单")
    ...
    # #         re = data.accounts_V2_loan(loanId=loanId, partnerId='AJK',loanAmount=loanAmount, prodType=prod_type,clientNo=self.clientNo,
    # #                                    userId=self.userId,repayMode=repayMode)
    # #         loger.info("test_4_02_还款方式不同的放款_还款方式为_2_等额本金_放款成功_安居客订单======返回报文：%s" % (re.text))
    # #         assert re.status_code == 200
    # #         res = re.json()
    # #         print(res)
    # #         assert res['code'] == 0
    ...
    # #         assert res['result'] == True
    # #     except (ZeroDivisionError, TypeError, NameError) as e:
    # #         loger.error("test_4_02_还款方式不同的放款_还款方式为_2_等额本金_放款成功_安居客订单====断言失败，失败原因：%s" % (e))

    # @pytest.mark.skip(reason="数据原因")
    # def test_5_01_接口必填参数校验_传入空订单号_接口调用失败_借据号不能为空_安居客订单(self):
    #     loger.info("=====test_5_01_接口必填参数校验_传入空订单号_接口调用失败_借据号不能为空_安居客订单=======接口测试开始")
    #     # loger.info("创建订单")
    #     # loanId = rep.commonParam.set_flow(count=2, describe='测试账单中心新接口放款接口-正常放款一百万以下的小额放款(/v2/loan)')  # 创建订单
    #     # loger.info("创建的订单号是：%s" % (loanId))
    #     # print("创建的订单号：", loanId)
    ...
    #         re = data.accounts_V2_loan(loanId=loanId, partnerId='AJK',loanAmount=loanAmount, prodType=prod_type, clientNo=self.clientNo,
    #                                    userId=self.userId)
    #         loger.info("test_5_01_接口必填参数校验_传入空订单号_接口调用失败_借据号不能为空_安居客订单======返回报文：%s" % (re.text))
    #         assert re.status_code == 200
    #         res = re.json()
    #         print(res)
    #         assert res['code'] != 0
    ...
    #
    #     except (ZeroDivisionError, TypeError, NameError) as e:
    #         loger.error("test_1_01_正常放款一百万以下的小额放款_传入贷款金额小于100万_可以正常放款且数据库中数据正常落库_安居客订单====断言失败，失败原因：%s" % (e))
    #
    # @pytest.mark.skip(reason="数据原因")
    # def test_5_02_接口必填参数校验_传入空userId_接口调用失败_用户id不能为空_安居客订单(self):
    #     loger.info("=====test_5_02_接口必填参数校验_传入空userId_接口调用失败_用户id不能为空_安居客订单=======接口测试开始")
    #     loger.info("创建订单")
    #     loanId = rep.commonParam.set_flow(count=2, describe='测试账单中心新接口放款接口-正常放款一百万以下的小额放款(/v2/loan)')  # 创建订单
    #     loger.info("创建的订单号是：%s" % (loanId))
    #     print("创建的订单号：", loanId)
    ...
    #         re = data.accounts_V2_loan(loanId=loanId, partnerId='AJK',loanAmount=loanAmount, prodType=prod_type, clientNo=self.clientNo,
    #                                    userId='')
    #         loger.info("test_5_02_接口必填参数校验_传入空userId_接口调用失败_用户id不能为空_安居客订单======返回报文：%s" % (re.text))
    #         assert re.status_code == 200
    #         res = re.json()
    #         print(res)
    #         assert res['code'] != 0
    ...
    #
    #     except (ZeroDivisionError, TypeError, NameError) as e:
    #         loger.error("test_5_02_接口必填参数校验_传入空userId_接口调用失败_用户id不能为空_安居客订单====断言失败，失败原因：%s" % (e))
    #
    # @pytest.mark.skip(reason="数据原因")
    # def test_5_03_接口必填参数校验_传入空clientNo_接口调用失败_clientNo不能为空_安居客订单(self):
    #     loger.info("=====test_5_03_接口必填参数校验_传入空clientNo_接口调用失败_clientNo不能为空_安居客订单=======接口测试开始")
    #     loger.info("创建订单")
    #     loanId = rep.commonParam.set_flow(count=2, describe='测试账单中心新接口放款接口-正常放款一百万以下的小额放款(/v2/loan)')  # 创建订单
    #     loger.info("创建的订单号是：%s" % (loanId))
    #     print("创建的订单号：", loanId)
    ...
    #     prod_type = rep.commonParam.prod_type(prod_type_desc='安居客')
    #     try:
    #         re = data.accounts_V2_loan(loanId=loanId, partnerId='AJK',loanAmount=loanAmount, prodType=prod_type, clientNo='',
    #                                    userId=self.userId)
    #         loger.info("test_5_03_接口必填参数校验_传入空clientNo_接口调用失败_clientNo不能为空_安居客订单======返回报文：%s" % (re.text))
    #         assert re.status_code == 200
    #         res = re.json()
    #         print(res)
    #         assert res['code'] != 0
    ...
    #
    #     except (ZeroDivisionError, TypeError, NameError) as e:
    #         loger.error("test_5_03_接口必填参数校验_传入空clientNo_接口调用失败_clientNo不能为空_安居客订单====断言失败，失败原因：%s" % (e))